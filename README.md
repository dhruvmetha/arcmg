...
1. A system `.py` file in `experiments/` that will get registered in `experiments/__init__.py` as a part of the `system_factory`. For example, `pendulum.py` is a system file. (<b>Note</b>: You can the modify the normalization functionality if you do not want to use [-1, 1] normalization by default, for example, check pendulum.py for a custom normalization example function.)
