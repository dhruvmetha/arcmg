...
class ReachabilityClassifier(nn.Module):
    def __init__(
        ...
        """
        Classifier to determine if target state is reachable from source state.
...
def create_classifier(config: Dict) -> ReachabilityClassifier:
    """Create classifier from config."""
    return ReachabilityClassifier(
